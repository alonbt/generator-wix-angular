...
Generator.prototype.createAppFile = function createAppFile() {
  ...
  if (this.name !== 'main') {
    this.angularModules = this.angularModules || "'" + this.scriptAppName + "Translations', '" + this.scriptAppName + "Preload', 'wixAngular', 'wix.common.bi', 'wixDashboardFramework'";
    this.appTemplate('app', 'scripts/' + this.name);
  } else if (this.env.options.dashboardApp || !this.env.options.dashboardWidget) {
    this.appTemplate('app', 'scripts/app');
    // this.appTemplate('mock/server-api', 'test/mock/server-api');
